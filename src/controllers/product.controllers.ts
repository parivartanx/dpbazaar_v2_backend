...
export class ProductController {
  ...
  private extractProductCardFields(product: any): any {
    ...
    const primaryImage =
      sanitized.images && sanitized.images.length > 0
        ? sanitized.images.find((img: any) => img.isPrimary) ||
          sanitized.images[0]
        : null;
    ...
    if (
      (mrp === null || mrp === undefined) &&
      sanitized.variants &&
      sanitized.variants.length > 0
    ) {
      const firstActiveVariant =
        sanitized.variants.find((v: any) => v.isActive !== false) ||
    ...
    const categoryNames = sanitized.categories
      ? sanitized.categories
          .map((cat: any) => cat.category?.name || cat.name)
          .filter(Boolean)
      : [];
    ...
    return {
      ...
      shortDescription:
        ...
        (sanitized.description
          ? sanitized.description.substring(0, 100)
          : null),
      mrp: mrp,
  ...
  private sanitizeProductForPublic(product: any): any {
    ...
    sanitizedProduct.sellingPrice = convertToNumber(
      sanitizedProduct.sellingPrice
    );
    ...
    if (sanitizedProduct.categories) {
      sanitizedProduct.categories = sanitizedProduct.categories.map(
        (cat: any) => {
          if (cat.category) {
            const {
    ...
    if (sanitizedProduct.variants) {
      sanitizedProduct.variants = sanitizedProduct.variants.map(
        (variant: any) => {
          ...
          sanitizedVariant.sellingPrice = convertToNumber(
            sanitizedVariant.sellingPrice
          );
  ...
  getAllProducts = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const { products, totalCount } =
        await this.repo.getAllWithFilters(filters);
      ...
      const productsWithConvertedNumbers = products.map(product => {
        ...
        if (converted.variants && Array.isArray(converted.variants)) {
          converted.variants = converted.variants.map((variant: any) => {
            ...
            convertedVariant.sellingPrice = convertToNumber(
              convertedVariant.sellingPrice
            );
      ...
      const transformedProducts =
        await this.imageUrlTransformer.transformCommonImageFields(
          productsWithConvertedNumbers
        );

      // Extract only essential fields for product cards/list view (optimized for performance)
      const productCards = transformedProducts.map(product =>
        this.extractProductCardFields(product)
      );
  ...
  getProductsFiltered = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      logger.info(
        `Found ${totalCount} products, returning ${products.length} products`
      );
      ...
      // Transform image keys to public URLs in the products response
      const transformedProducts =
        await this.imageUrlTransformer.transformCommonImageFields(products);
  ...
  getProductBySlug = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const sanitizedProduct =
        this.sanitizeProductForPublic(transformedProduct);
  ...
  /**
   * Create a complete product with all related entities (images, variants, attributes, inventory)
   * in a single atomic transaction.
   */
  createProductComplete = async (
    req: Request,
    res: Response
  ): Promise<void> => {
    try {
      logger.info(
        'createProductComplete - Request body:',
        JSON.stringify(req.body, null, 2)
      );
      const product = await this.repo.createComplete(req.body);
      ...
      res.status(201).json({
        success: true,
        message: 'Product created successfully with all related entities',
        data: { product: transformedProduct },
        timestamp: new Date().toISOString(),
      });
    } catch (error: any) {
      logger.error(`error in createProductComplete: ${error}`);
      ...
      const response: ApiResponse = {
        success: false,
        error: (error as Error).message,
        message: 'Problem in Creating Complete Product',
        timestamp: new Date().toISOString(),
      };
      res.status(500).json(response);
    }
  ...
  updateProduct = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // Transform image keys to signed URLs in the product response
      const transformedProduct =
        await this.imageUrlTransformer.transformImageKeysToSignedUrls(product);
  ...
  addImage = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const image = await this.repo.addProductImage(
        productId,
        imageKey,
        isPrimary
      );
      ...
      const transformedImage =
        await this.imageUrlTransformer.transformCommonImageFields(image);
  ...
  addImagesBulk = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const transformedImages =
        await this.imageUrlTransformer.transformCommonImageFields(images);
  ...
  getProductVariants = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const transformedVariants =
        await this.imageUrlTransformer.transformCommonImageFields(variants);
  ...
  createVariant = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const transformedVariant =
        await this.imageUrlTransformer.transformCommonImageFields(variant);
    ...
    try {
      ...
      if (error.code === 'P2003') {
        res.status(400).json({
          ...
          message:
            'Variant cannot be deleted because it has associated inventory or order items. Please remove or reassign them first.',
          timestamp: new Date().toISOString(),
        });
        return;
      }
    ...
    try {
      ...
      if (error.code === 'P2003') {
        res.status(400).json({
          ...
          message:
            'Attribute cannot be deleted because it has associated products or categories. Please remove associations first.',
          timestamp: new Date().toISOString(),
        });
        return;
      }
